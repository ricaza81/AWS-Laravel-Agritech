...
//login
        ...
        protected function landing()
    {
        ...
        ->with("departamentos",$departamentos)
         ->with("cultivos",$cultivos);
            ...
            protected function restaurantes()
    {

        $asesoresagronielsen=AsesoresAgronielsen::orderBy('id', 'desc')->paginate(3);
        $asesorestotales=AsesoresAgronielsen::all();
        $paises=Pais::all();
        $msj="";
        $cultivos=Cultivos::all();
        $zonas=Zonas::all();
        $canales=Canales::all();
        $departamentos=Departamentos::all();
        $productores=DirectoAgricultor::orderBy('id', 'desc')->paginate(3);
        $agricultores=User::all();

        return view('directofinca.restaurantes')
        ->with("asesoresagronielsen",$asesoresagronielsen)
        ->with("asesorestotales",$asesorestotales)
        ->with("paises",$paises)
        ->with("msj",$msj)
        ->with("zonas",$zonas)
        ->with("canales",$canales)
         ...
         protected function consultaclima()
    {
        ...
        return view('landing.consultaclima')
           ...
           protected function iottempenture()
    {
        ...
        ->with("departamentos",$departamentos)
         ->with("cultivos",$cultivos);
            ...
            protected function directofinca()
    {
        $productorecomendado = ProductoRecomendado::where('id','=',2)->latest()->first();
        $paises=Pais::all();
        $cultivos=Cultivos::all();
        ...
        //$productores=AgriAgricultores::all();
        //$productores=User::orderby('id','desc')->get();
        //$productores=DirectoAgricultor::all();
        $productores=DirectoAgricultor::orderby('id','desc')->get();
        //$productores=AgriAgricultores::all()->orderBy('id', 'desc');//->paginate($pagination);

        return view('directofinca.landing')
        ->with("paises",$paises)
        ->with("productorecomendado",$productorecomendado)
        ->with("zonas",$zonas)
        ->with("canales",$canales)
             ...
             protected function tienda()
     {
      ...
      //    $usuarioactual=18;
        ...
        return view('directofinca.tienda')->with([
            'products' => $products,
  ...
  protected function calculo_req_cultivo()
    ...
    protected function demoalmacenes()
    {
        ...
        return view('landing.demoalmacenes')
        ->with("paises",$paises)
        ->with("zonas",$zonas)
        ->with("canales",$canales)
        ->with("departamentos",$departamentos)
        ...
        protected function analiticacultivos()
    {
        ...
        return view('landing.analiticacultivos')
        ->with("paises",$paises)
        ->with("zonas",$zonas)
        ->with("canales",$canales)
        ->with("departamentos",$departamentos)
    ...
    if ($this->auth->attempt($credentials, $remember))
        ...
        /*Redirecciones por condiciones*/
                  ...
                  $cantpdtossinprecio=count($prdtossinprecio);
                    ...
                    /*if ($cantpdtos>0) {return redirect('productoresoferta');}
                    if(\Auth::user()->id==1110528441){return redirect('productoresoferta');}
                    //if ($cantpdtos==0) {return redirect('formnuevoproductoempresa');}
                    if ($cantfincascompartidas>0) {return redirect('productoresoferta');}
                    /*condicion 2: la empresa tiene productos creados pero al menos hay uno que no tiene precio*/
                    /*if ($cantpdtos>0 && $cantpdtossinprecio>0) {return redirect('productoresoferta');}
                    /*condicion 3: la empresa tiene productos creados. No importa ninguna otra variable*/
                    /*if ($cantpdtos>0 && $cantpdtossinprecio=0)*/
